...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        let input_splitted: Vec<String> = s.split(',').map(|s| s.to_string()).collect();

        if input_splitted.len() != 2 {
            return Person::default();
        }

        if input_splitted[0].is_empty() {
            return Person::default();
        }

        let age = input_splitted[1].parse::<u8>();
